...
## Notes On Removed/Generated Files

The old variance-reduction setting sweep and obsolete local `tests/` directory
were removed to keep the current EAST triangular workflow focused.  Use the
`setting_Trimesh_D_1.log` through `setting_Trimesh_D_9.log` files for the
current no-VR sweep.

Temporary directories such as `.vr_sync` and `numcheck_work` are not part of the
runtime model and can be removed when present.
