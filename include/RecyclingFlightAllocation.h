	...
	double atomic_source, double molecular_source, int total_flights)
{
	...
	// EIRENE test-particle weights are equivalent atomic fluxes. A hydrogenic
	// molecule therefore contributes two nuclei when sharing a history budget.
	const double molecular_atomic_source = 2.0 * molecular_source;
	const double total_atomic_source = atomic_source + molecular_atomic_source;
	if (total_flights <= 0 || total_atomic_source <= 0.0)
		return {0, 0};
	if (atomic_source <= 0.0)
		return {0, total_flights};
	if (molecular_atomic_source <= 0.0)
		return {total_flights, 0};

	int atomic_flights = static_cast<int>(std::llround(
		total_flights * atomic_source / total_atomic_source));
